    ...
    *,
    ...
    try:
        with pd.ExcelWriter(temp_path, engine="openpyxl") as writer:
            ...
            report_kind = next(
                (
                    str(frame.attrs.get("toolbox_report_kind"))
                    for frame in tables.values()
                    if str(frame.attrs.get("toolbox_report_kind") or "").strip()
                ),
                "excel_data_toolbox_export",
            )
            report_uuid = str(uuid.uuid4())
            writer.book.properties.identifier = f"urn:uuid:{report_uuid}"
            writer.book.properties.subject = "Generated by Excel Data Toolbox AI"
            writer.book.properties.keywords = (
                "excel-data-toolbox-ai;generated-workbook;schema=1;"
                f"report_kind={report_kind};report_uuid={report_uuid}"
            )
